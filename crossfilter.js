...
crossfilter_bitarray.prototype.add = function() {
  ...
  for (var i = 0; i < this.subarrays; i++) {
    ...
    if (w < 32 && (one & -(1 << w))) {
      // widen this subarray
      crossfilter_arrayWiden(this[i], w <<= 1);
      this.width = 32 * i + w;
    }
  ...
  // add a new subarray
  this[this.subarrays] = crossfilter_array8(0);
...
function crossfilter() {
  ...
  function dimension(value) {
    ...
    function preAdd(newData, n0, n1) {
      ...
      if (refilterFunction) {
        for (i = 0; i < n1; ++i) {
          if (!refilterFunction(newValues[i], k = newIndex[i] + n0)) filters[offset][k] |= one;
        }
      } else {
    ...
    function filterIndexFunction(f) {
      ...
      for (i = 0; i < n; ++i) {
        if (!(filters[offset][k = index[i]] & one) ^ (x = f(values[i], k))) {
    ...
    function remove() {
      ...
      if (i >= 0) dataListeners.splice(i, 1);
      for (i = 0; i < n; ++i) filters[offset][i] &= zero;
